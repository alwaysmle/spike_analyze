# spike_analyze
### Abstract
The growth and demise of the dendritic spine is critical in neurobiology, which has high correlation to the formation of memory. Traditionally, the researcher labeled the dendritic spines manually, and the existing automatic labeling methods are often only suitable for in vitro brain slice images captured by confocal microscopy with high resolution. The brain image of the living brain captured by the two-photon microscope cannot not be automatically analyzed pefectly due to 1. environmental noises and 2. low resolution in relatively deep area of brain. As a result, this article aims to propose a set of algorithms for automatic spine marking, which can effectively detect the location and approximate classification of most spines.
### Method
